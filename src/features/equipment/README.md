...
Recorded rather than fixed, because the fix is a routing or product decision
rather than a migration one:

- **`BuildingList` and `BuildingDetail` resolve `customers-building-*`** when the
  member has no branches. No router defines those names. Both screens are
  mounted only by `router/equipment.js`.
- **`EquipmentList` emits `customers-equipment-view-{type}` and
  `settings-equipment-view`/`-edit`** (untyped) on some tenant and role
  combinations. The typed variants exist only under the `equipment` and
  `settings` prefixes and the untyped ones only under `customers`.
- **`LocationView`'s equipment table links through
  `route_prefix.replace('location', 'equipment') + '-view'`**, which on a
  settings mount yields a name that is not registered.
- **The equipment QR export forwards only `q`, not the equipment type**, so the
  spreadsheet covers every type.
