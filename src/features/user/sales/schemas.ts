 ...
 *
 * The generated create body accepts blanks the legacy vuelidate `required`
 * rules rejected, so the form adds minLength(1) to username, email and the
 * names. `contract_hours_week` keeps the generator's decimal regex; the
 * legacy text input posted digit strings DRF coerced, so the parse coerces
 * here the same way. Passwords are create-only client fields (see below).
 */
...
export interface SalesUserFormValues {
  username: string
  first_name: string
  last_name: string
  email: string
...
export type SalesUserFieldErrors = Partial<Record<
 ...
 * The username probe verdict arrives separately (see use-username-probe.ts) —
 * this function only validates what the fields themselves say.
 */
export function validateSalesUserForm(
  values: SalesUserFormValues,
  { isCreate }: { isCreate: boolean },
): SalesUserFieldErrors {
  const result = v.safeParse(salesUserFormSchema, {
  ...
  const errors: SalesUserFieldErrors = {}
  if (!result.success) {
    for (const issue of result.issues) {
      const field = issue.path?.[0]?.key as keyof SalesUserFieldErrors | undefined
...
export function parseSalesUserForm(
  ...
  const parsed = v.parse(salesUserFormSchema, {
    username: values.username,
    email: values.email,
    first_name: values.first_name,
    last_name: values.last_name,
