...
describe('EquipmentList QR export', () => {
  ...
  test('exports the committed search term, encoded', async () => {
    ...
    expect(download).toHaveBeenCalledWith('/api/equipment/equipment-export-qr/?q=ketel+%26+pomp', 'equipment.xlsx')
  })
  ...
  test('commits the search draft before exporting, so the file answers the screen', async () => {
    ...
    expect(download).toHaveBeenCalledWith('/api/equipment/equipment-export-qr/?q=ketel', 'equipment.xlsx')
  })

  test('an unfiltered export sends no search term', async () => {
    const download = spyDownload()
    const wrapper = await mountEquipment()

    await wrapper.get('button[title="Download QR-codes"]').trigger('click')

    expect(download).toHaveBeenCalledWith('/api/equipment/equipment-export-qr/?', 'equipment.xlsx')
  })
})
