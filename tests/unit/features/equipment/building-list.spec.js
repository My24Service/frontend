...
// The screen's links are its own: it takes no route_prefix and names the two
// route families itself. The customers-building-* names are registered here so
// the branchless arm is observable, not because they exist in the application.
const routes = [
  ...
  {name: 'customers-building-add', path: '/customers/buildings/form', component: {template: '<div />'}},
  {name: 'customers-building-edit', path: '/customers/buildings/form/:pk', component: {template: '<div />'}},
  {name: 'customers-building-view', path: '/customers/buildings/:pk', component: {template: '<div />'}},
...
describe('BuildingList', () => {
  ...
  test('a branchless tenant asks for the customers-building names, as the legacy screen did', async () => {
    // Those names are registered nowhere in src/router - a pre-existing defect
    // recorded in the module README rather than changed by this migration.
    const wrapper = await mountBuildings({main: {getMemberHasBranches: false}})

    expect(wrapper.get('tbody a[href="/customers/buildings/31"]').exists()).toBe(true)
    expect(wrapper.get('a.btn').attributes('href')).toBe('/customers/buildings/form')
  })
