...
describe('PlanningUserList URL mirroring', () => {
  test('a shared address restores the view, page included, before the first request', async () => {
    ...
    expect(wrapper.get('input[aria-label="Search planning users"]').element.value).toBe('jan')
  })
  ...
  test('the settings mount restores the same address shape', async () => {
    ...
    await mountPlanningList({ props: { fromSettings: true } })
